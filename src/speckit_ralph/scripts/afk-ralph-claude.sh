...
detach_loop() {
  ...
  LOG_FILE="${LOG_FILE:-$REPO_ROOT/logs/ralph-claude-$TIMESTAMP.log}"
  PID_FILE="${PID_FILE:-$REPO_ROOT/logs/ralph-claude.pid}"
...
ARTIFACT_DIR="${RALPH_ARTIFACT_DIR:-$(mktemp -d -t ralph-claude-loop.XXXXXX)}"
mkdir -p "$ARTIFACT_DIR"

# Cleanup artifacts unless user explicitly wants to keep them
cleanup_artifacts() {
...
# Check if iteration completed with promise
check_promise() {
  if [[ -f "$ARTIFACT_DIR/output.txt" ]] && \
    grep -q "<promise>${PROMISE}</promise>" "$ARTIFACT_DIR/output.txt"; then
